...
1. `tsconfig.json`：以上游 `tsconfig.upstream.json` 为基础，仅把 `outDir` 改为 `./dev-dist`，
   使根命令构建到**非运行时**的开发输出；`dist/` 永不作为构建目标。
2. `scripts/lib/cross-platform-command.mjs`：跨平台 child-process 调用助手。契约是
   ...
   `runSync()` 供真实调用。`build-dev.mjs` 的 `tsc` 与 `tests/baseline-boundary.test.ts`
   的 `npm run build` 都经它执行。
3. `scripts/build-dev.mjs`：`npm run build:quota-zh` 的入口 —— 清理 `dev-dist`、经
   ...
   上游的 esbuild/babel TUI 打包（`prepare-tui-dist.mjs`）属于运行时管线，不属于本开发构建。
4. `vitest.config.ts`：离线测试基线配置（`root` 指向本目录，`include: tests/**/*.test.ts`）。
...
5. `tests/setup.ts`：离线测试网络守卫 —— 对全局 `fetch` 与 `node:http`/`https`/`net`/`tls`/
...
6. `scripts/stage-runtime.mjs`：quota-zh 的 runtime staging 只允许
   ...
   拒绝，当前已安装的中文版 Server/TUI/命令运行时文件与行为不变。
7. `tests/baseline-boundary.test.ts`：Ticket 01 边界验证 —— 源码基线存在性与可追溯性、开发构建
   不触碰 `dist/`、staging 不携带非运行时文件、上游 `CONTRIBUTING.md` 仅以
   `references/upstream-quota/CONTRIBUTING.md` 参考形式存在（插件根级不存在贡献者入口）。
   child-process 调用断言直接验证 `{ command, shell }` 语义，不依赖对脚本源码的文本匹配。

## 与当前中文版运行时的关系

本基线是**独立开发基线**。当前 `dist/` 内已安装的中文版产物、`package.json` 入口、TUI/Server
行为均不因本 Ticket 改变；后续 Ticket 从 `src/` 出发重放中文行为，再生成新的运行时产物。

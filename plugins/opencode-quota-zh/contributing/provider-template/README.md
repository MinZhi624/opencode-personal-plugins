...
Use `Existing OpenCode auth, global config, or env` only after manual verification confirms all three paths work. This plugin has no automated test suite (see `AGENTS.md`); do not add test files or a test framework.

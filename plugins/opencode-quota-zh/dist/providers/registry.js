 ...
 */
import { QUOTA_PROVIDER_REGISTRATION_SOURCE, } from "../lib/provider-registration.js";
...
export function getProviders() {
    return QUOTA_PROVIDER_REGISTRATION_SOURCE.map(({ id }) => {
        ...
        if (provider.id !== id) {
            throw new Error(`Quota provider registration mismatch: expected ${id}, received ${provider.id}`);
        }

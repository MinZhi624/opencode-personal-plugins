...
export const QUOTA_ALERT_EPISODES_STATE_VERSION = 1;
...
export function buildQuotaAlertEpisodeId(params) {
    const windowPart = params.metricType === "percent_remaining"
        ? (params.windowLabel ?? "")
...
function candidateToEpisodeId(candidate) {
...
function episodeFromCandidate(candidate, nowIso) {
    return {
...
function lastNotifiedMs(episode) {
...
export function transitionAlertEpisodes(params) {
    ...
    for (const episode of params.episodes) {
        ...
        const isRepeatEligible = repeatMs !== null && nowMs - lastNotifiedMs(episode) >= repeatMs;
        if (isRepeatEligible) {
            const next = {
                ...episode,
                severity: candidate.severity,
    ...
    for (const candidate of params.candidates) {
        ...
        if (params.episodes.some((episode) => episode.resolvedAtIso === undefined && episode.episodeId === episodeId)) {
            continue;
        }
 ...
 */
export function pruneQuotaAlertEpisodes(episodes, now) {
    const cutoffMs = now.getTime() - RESOLVED_EPISODE_RETENTION_MS;
    const kept = episodes.filter((episode) => episode.resolvedAtIso === undefined || Date.parse(episode.resolvedAtIso) >= cutoffMs);
    ...
    const resolvedKept = [...resolved]
        .sort((left, right) => (right.resolvedAtIso ?? "").localeCompare(left.resolvedAtIso ?? ""))
 ...
 */
export function getQuotaAlertEpisodesPath() {
...
function isValidEpisode(value) {
    ...
    if (value.windowLabel !== undefined && typeof value.windowLabel !== "string")
        return false;
    if (value.currency !== undefined && typeof value.currency !== "string")
        return false;
    if (value.status !== undefined && value.status !== "unavailable")
        return false;
 ...
 */
export async function readQuotaAlertEpisodes(path) {
 ...
 * infers anything.
 */
export async function writeQuotaAlertEpisodes(episodes, options = {}) {
    const state = {
...
function formatClockTime(iso, now) {
    if (!iso)
        return null;
 ...
 */
export function formatQuotaAlertNotificationText(notification) {
    const base = notification.metricType === "percent_remaining"
        ? `额度告警：${notification.providerLabel}${notification.windowLabel ? ` ${notification.windowLabel}` : ""} 额度剩余 ${Math.round(notification.percentRemaining ?? 0)}%`
        : notification.metricType === "balance"
            ? `额度告警：${notification.providerLabel} 余额 ${formatAmountValue(notification.amount ?? 0, notification.currency)}`
            : `额度告警：${notification.providerLabel} 当前不可用`;
    const thresholdPart = notification.metricType === "percent_remaining" &&
        ...
        : notification.metricType === "balance" &&
            ...
            : "";
    const repeatPart = notification.notifyCount > 1 ? `（第 ${notification.notifyCount} 次提醒）` : "";
...
export function buildQuotaAlertsReport(params) {
    ...
    const metricText = (episode) => episode.metricType === "percent_remaining"
        ? `${episode.windowLabel ? `${episode.windowLabel} ` : ""}百分比额度`
        : episode.metricType === "balance"
            ? `${episode.currency ?? ""} 余额`
            : "可用状态";
    const severityText = (episode) => episode.severity === "critical" ? "严重" : "警告";
    ...
    if (active.length > 0) {
        ...
        for (const episode of active) {
            ...
            lines.push(`- ${episode.providerLabel} · ${metricText(episode)} · ${severityText(episode)}` +
    ...
    if (recovered.length > 0) {
        ...
        for (const episode of recovered) {
            ...
            lines.push(`- ${episode.providerLabel} · ${metricText(episode)} · 已恢复 ${resolvedAt ?? episode.resolvedAtIso}`);
        }
    }

...
import { cloneQuotaProviderResult, decodePersistedQuotaProviderCacheEntry, encodePersistedQuotaProviderCacheEntry, normalizeQuotaProviderResult, } from "./quota-state-codec.js";
...
function getQuotaProviderStateCacheLocator(key) {
    return createHash("sha256").update(key).digest("hex");
}
function getQuotaProviderCacheFileStem(providerId) {
...
export function getQuotaProviderStateCacheFilePath(providerId, key) {
    return join(getQuotaProviderCacheDir(), `${getQuotaProviderCacheFileStem(providerId)}-${getQuotaProviderStateCacheLocator(key)}.json`);
}
...
async function writePersistedQuotaProviderCacheEntry(entry, logicalKey) {
    try {
...
async function fetchValidatedProviderResult(provider, ctx, cacheContext) {
...
async function resolveProviderCacheScope(provider, ctx, cacheContext) {
    const policy = provider.cachePolicy;
    if (!policy || policy.kind === "uncached")
        return null;
    if (policy.kind === "account-neutral")
        return {};
    try {
...
async function isProviderCacheScopeCurrent(provider, ctx, scope) {
    if (provider.cachePolicy?.kind === "account-neutral")
        return true;
    if (provider.cachePolicy?.kind !== "resolved-auth")
        return false;
    try {
        ...
        if (runtimeEligibleQuotaProviders === null)
            return false;
        return ((await provider.cachePolicy.resolveIdentity(ctx, { runtimeEligibleQuotaProviders })) ===
...
function getProcessLocalRuntimeOwner(ctx) {
...
export async function fetchQuotaProviderResult(params) {
    ...
    const fetchUncached = async (cacheContext = {}) => {
    ...
    if (runtimeEligibleQuotaProviders === null)
        return fetchUncached();
    const cacheContext = { runtimeEligibleQuotaProviders };
    const scope = await resolveProviderCacheScope(provider, ctx, cacheContext);
    if (!scope)
        return fetchUncached(cacheContext);

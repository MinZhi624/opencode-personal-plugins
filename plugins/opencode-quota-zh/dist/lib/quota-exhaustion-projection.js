...
export function isFixedWindowProjectionEvidence(value, resetTimeIso) {
    if (!value || typeof value !== "object" || Array.isArray(value))
        return false;
    const evidence = value;
    if (Object.keys(evidence).some((key) => !["kind", "startedAtIso", "observedAtIso", "endsAtIso", "fullReset"].includes(key)) ||
    ...
    if (startedAt === null || observedAt === null || endsAt === null)
        return false;
    if (!(startedAt < observedAt && observedAt < endsAt))
        return false;
    const resetAt = parseIsoTimestamp(resetTimeIso);
    if (resetAt === null || resetAt !== endsAt)
        return false;
    return true;
}
export function projectQuotaRunway(params) {
    const nowMs = params.nowMs ?? Date.now();
    if (!Number.isFinite(params.percentRemaining) || !Number.isFinite(nowMs))
        return null;
    if (!isFixedWindowProjectionEvidence(params.fixedWindow, params.resetTimeIso))
        return null;
    ...
    if (nowMs < observedAt || nowMs >= endsAt)
        return null;
...
function formatRunwayDuration(milliseconds) {
...
export function formatQuotaRunway(projection, nowMs = Date.now()) {
    if (!projection)
        return "";
    if (projection.kind === "lasts_past_reset")
        return "lasts past reset";
    const projectedAt = parseIsoTimestamp(projection.projectedAtIso);
    if (projectedAt === null || !Number.isFinite(nowMs))
        return "";
    return `≈ ${formatRunwayDuration(projectedAt - nowMs)}`;
}
export function compareQuotaRunwayUrgency(left, right, nowMs = Date.now()) {
    const urgency = (projection) => {
        if (!projection)
        ...
        if (projectedAt === null)
            return [3, Number.POSITIVE_INFINITY];
        if (projectedAt <= nowMs)
            return [0, projectedAt];

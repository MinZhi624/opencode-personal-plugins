...
async function loadSidebar(api: TuiPluginApi, sessionID: string): Promise<SidebarState> {
...
export function ChineseSidebarContentView(props: { api: TuiPluginApi; sessionID: string }) {
  ...
  const reload = () => {
    ...
    inFlight = true;
    loadSidebar(props.api, props.sessionID)
      .then(setState)
      .catch((error) => {
        setState({
          status: "error",

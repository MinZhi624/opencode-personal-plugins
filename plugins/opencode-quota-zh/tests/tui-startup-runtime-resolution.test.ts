...
const collectQuotaRenderDataMock = vi.hoisted(() => ({ collectQuotaRenderData: null as unknown as ReturnType<typeof vi.fn> }));
...
vi.mock("../src/lib/quota-render-data.js", async (importOriginal) => {
  ...
  const mocked = vi.fn(async (params: { config: { tuiCompactStatus: { maxWidth: number } } }) => ({
    active: [],
    data: {
      entries: [
        {
          ...
          name: `generation-${params.config.tuiCompactStatus.maxWidth - 40}`,
          group: "Marker",
          label: `generation-${params.config.tuiCompactStatus.maxWidth - 40}`,
          value: "marker",
        },
      ],
      errors: [],
  ...
  collectQuotaRenderDataMock.collectQuotaRenderData = mocked;
  return {
    ...actual,
    collectQuotaRenderData: mocked,
  };
});
...
async function startScenario(controlledDelayMs: number): Promise<StartedScenario> {
  ...
  const sidebarRegistration = registered.find((entry) => entry.order === 40);
  ...
  const observeGeneration = (generation: number) => {
    ...
    // Chinese fork: the sidebar_content slot mounts the Chinese sidebar
    // (per-instance state, collectQuotaRenderData path) whose cards cannot be
    // observed through re-renders without reactivity. Its data path is
    // observed through the config generation passed to collectQuotaRenderData.
    const sidebarGenerations = collectQuotaRenderDataMock.collectQuotaRenderData.mock.calls.map((call) =>
      configGeneration((call[0] as { config: unknown }).config),
    );
    const observed = {
      sidebar: sidebarGenerations.includes(generation),
...
describe("TUI startup runtime/config resolution", () => {
  ...
  it("shares the initial sidebar and compact load, then rereads config on mount recovery", async () => {
    ...
    try {
      ...
      // Session resource recovery (500 ms) rereads generation 2.
      expect(scenario.observeGeneration(2)).toMatchObject({ compact: true });
      expect(
      ...
      // Chinese fork: the sidebar_content slot runs its own recovery cadence
      // (600 ms timer) and rereads generation 2 through collectQuotaRenderData.
      await scenario.advanceTo(scenario.mountAtMs + 600);
      await scenario.advanceTo(scenario.mountAtMs + 625);

...
function isForbidden(file) {
  ...
  // Local deviation from upstream v4.4.1: the upstream allowlist targets
  // references/upstream-plugins/ (third-party plugin snapshots). That tree is not
  // part of this bundle's baseline (see UPSTREAM-PROVENANCE.md); the only tracked
  // reference retained here is the verbatim upstream document reference
  // references/upstream-quota/CONTRIBUTING.md, so the allowlist follows it.
  if (isWithin(file, "references")) {
    return !file.startsWith("references/upstream-quota/");
  }

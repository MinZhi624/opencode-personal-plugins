...
export class OrchestrationManager {
    ...
    async submitSlice(raw, parentSessionId, directory) {
        ...
        for (const task of this.tasks.values())
            if (task.kind === "slice" && !["failed", "cancelled"].includes(task.runStatus) && task.acceptanceStatus === "not_evaluated") {
    ...
    async launch(task) {
        ...
        try {
            ...
            const created = await this.client.session.create({ body: { parentID: task.parentSessionId, title: `${task.kind === "slice" ? "Slice" : "Assignment"}: ${task.taskId}`, permission }, query: { directory: task.directory } });
            if (!created.data?.id)
                throw new Error(`Failed to create Worker session: ${created.error ?? "missing session"}`);

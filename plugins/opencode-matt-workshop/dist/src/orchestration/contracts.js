...
export const sliceSpecSchema = z.object({
    ...
    if (value.budgets.turnWarning >= value.budgets.turnLimit)
        context.addIssue({ code: "custom", path: ["budgets", "turnWarning"], message: "turn warning must be below limit" });
    if (value.budgets.wallWarningMinutes >= value.budgets.wallLimitMinutes)
        context.addIssue({ code: "custom", path: ["budgets", "wallWarningMinutes"], message: "wall warning must be below limit" });
    if (Buffer.byteLength(JSON.stringify(value), "utf8") > 256 * 1024)
        context.addIssue({ code: "custom", message: "initial Slice context exceeds 256 KiB" });
    for (const [field, paths] of [["writeSet", value.writeSet], ["allowedTestPaths", value.testBudget.allowedTestPaths], ["contextRefs", value.contextRefs]]) {
        paths.forEach((path, index) => {
            if (path.startsWith("/") || path.split(/[\\/]/).includes(".."))
                context.addIssue({ code: "custom", path: [field, index], message: "path must stay relative to the Slice worktree" });
        });
    }
});
export const assignmentSpecSchema = z.object({
    ...
    if (Buffer.byteLength(JSON.stringify(value), "utf8") > 256 * 1024)
        context.addIssue({ code: "custom", message: "initial Assignment context exceeds 256 KiB" });
    for (const [index, root] of value.externalReadRoots.entries()) {
        const absolute = resolve(root);
        if (absolute === "/" || absolute === resolve(homedir()))
            context.addIssue({ code: "custom", path: ["externalReadRoots", index], message: "external read root is too broad" });
    }
});

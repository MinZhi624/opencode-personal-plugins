...
const git = async (cwd: string, args: string[]) => (await exec("git", args, { cwd, maxBuffer: 4 * 1024 * 1024 })).stdout.trim()
const safeId = (value: string) => value.replace(/[^a-zA-Z0-9._-]/g, "-").slice(0, 80)

export type IntegrationWorkspace = { root: string; originalBranch: string; baseline: string; branch: string; directory: string }

export class GitWorkspace {
  private integrations = new Map<string, IntegrationWorkspace>()

  async integration(parentSessionId: string, directory: string): Promise<IntegrationWorkspace> {
    const existing = this.integrations.get(parentSessionId)
    if (existing) return existing
    const root = await git(directory, ["rev-parse", "--show-toplevel"])
    if (await git(root, ["status", "--porcelain"])) throw new Error("Workshop implementation requires a clean Git worktree")
  ...
  async changedPaths(directory: string) {
  ...
  async addedPaths(directory: string) {
    const added = await git(directory, ["diff", "--name-only", "--diff-filter=A", "HEAD"])
    const untracked = await git(directory, ["ls-files", "--others", "--exclude-standard"])
    return [...new Set([...added.split("\n"), ...untracked.split("\n")].filter(Boolean))]
  }

  async accept(directory: string, integration: IntegrationWorkspace, sliceId: string, ticketRef?: string) {
    await git(directory, ["add", "-A"])
    if (!await git(directory, ["diff", "--cached", "--name-only"])) throw new Error("Cannot accept a Slice with no changes")
  ...
  async summary(parentSessionId: string) {
    const integration = this.integrations.get(parentSessionId)
    if (!integration) throw new Error("No Integration Branch for this Foreman session")
  ...
  async finalize(parentSessionId: string) {
    const integration = this.integrations.get(parentSessionId)
    if (!integration) throw new Error("No Integration Branch for this Foreman session")
    if (await git(integration.root, ["status", "--porcelain"])) throw new Error("Final integration blocked: the user's original worktree is dirty")
    const currentBranch = await git(integration.root, ["rev-parse", "--abbrev-ref", "HEAD"])
    if (currentBranch !== integration.originalBranch) throw new Error(`Final integration blocked: expected original branch ${integration.originalBranch}`)
    const originalTip = await git(integration.root, ["rev-parse", "HEAD"])
    if (originalTip !== integration.baseline) throw new Error("Final integration blocked: original branch advanced; create a new Final Integration flow")
  ...
  async removeWorktree(root: string, directory: string) {

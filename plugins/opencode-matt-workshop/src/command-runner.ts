...
type CommandLimits = { memory: string; swap: string; cpuQuota: string; tasks: number; timeoutMs: number }
...
export class CommandRunner {
  ...
  preflight() {
    return this.preflightPromise ??= (async () => {
      if (process.platform !== "linux") throw new Error("unsupported_runtime: controlled commands require Linux or WSL")
      await access("/sys/fs/cgroup/cgroup.controllers")
      const controllers = await readFile("/sys/fs/cgroup/cgroup.controllers", "utf8")
      for (const required of ["cpu", "memory", "pids"]) if (!controllers.split(/\s+/).includes(required)) throw new Error(`unsupported_runtime: cgroup v2 ${required} controller is unavailable`)
      await promisify(execFile)("systemd-run", ["--user", "--quiet", "--wait", "--collect", "true"], { timeout: 5_000 })
    })()
  }
  classify(command: string): "allow" | "ask" | "deny" {
    if (denied.some((pattern) => pattern.test(command))) return "deny"
    if (approval.some((pattern) => pattern.test(command))) return "ask"
    return "allow"
  }

  async run(command: string, cwd: string, limits: CommandLimits, signal: AbortSignal): Promise<CommandResult> {

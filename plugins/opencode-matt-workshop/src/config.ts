...
function skillsPath() {
  ...
  if (existsSync(join(match, "..", ".matt-sync-in-progress"))) throw new Error("opencode-matt-workshop: incomplete Matt skill sync detected")
  return match
}

export const WORKSHOP_SKILLS_PATH = skillsPath()

export function applyWorkshopConfig(config: any, options: any) {
  config.agent ??= {}
  if (options.replace_builtin_agents) {
    for (const id of replacedAgents) config.agent[id] = { ...(config.agent[id] ?? {}), disable: true }
  }
  Object.assign(config.agent, buildWorkshopAgents(options))
  config.default_agent = "tinker"
  config.skills ??= {}
  const paths = config.skills.paths ?? []
  if (!paths.includes(WORKSHOP_SKILLS_PATH)) config.skills.paths = [...paths, WORKSHOP_SKILLS_PATH]
  config.command ??= {}
